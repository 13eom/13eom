<div class="ft_blank"></div>
<footer>
  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 425 383" fill="none" class="logo">
    <path d="M153.209 57.2071C132.008 138.887 151.365 153.248 214.966 103.881L240.775 83.237L218.653 42.8458C186.392 -17.2923 171.644 -14.5995 153.209 57.2071Z" fill="#5F2C29"/>
    <path d="M295.159 78.7487C238.01 118.242 181.783 153.248 170.722 155.941C158.739 158.634 137.539 153.248 119.104 142.477C75.7812 116.447 42.598 118.242 19.5541 146.965C8.49306 160.429 0.197266 178.38 0.197266 188.254C0.197266 202.615 4.80604 204.41 30.6152 199.922C66.5637 192.742 84.077 208.898 99.7468 263.651C114.495 313.018 131.087 324.686 184.548 323.789C221.419 322.891 230.636 326.481 238.01 342.638C242.619 353.409 258.289 367.77 272.115 374.951C294.237 386.62 299.768 385.722 330.186 370.463C368.899 349.819 390.1 313.915 381.804 281.602C377.195 262.753 371.665 260.06 335.716 260.958C282.254 260.958 262.898 244.801 308.064 236.723C352.308 228.645 392.865 185.561 410.378 130.808C427.892 72.4656 428.814 6.94206 412.222 6.94206C405.77 6.94206 353.23 39.255 295.159 78.7487Z" fill="#5F2C29"/>
  </svg>

  <div class="container">
    <div class="inner">
      <a href="/" class="logo"></a>
      <ul class="ft_menu">
        <li>
          <ul>
            <li>
              Mail to
              <a href="mailto:dltmdqja6563@gmail.com">dltmdqja6563@gmail.com</a>
            </li>
            <li>
              Tel
              <a href="tel:01050599920">01050599920</a>
            </li>
            <li><a href="instagram.com/_13eom/" target="_blank">/ Insta</a></li>
            <li><a href="soundcloud.com/13eom" target="_blank">/ Soundcloud</a></li>
          </ul>
        </li>
        <li class="pc">
          <ul>
            <li><a href="/index.php">Birdman</a></li>
            <li><a href="/index.php#main_best_pf">Best portfolio</a></li>
            <li><a href="/index.php#main_feature">Features</a></li>
            <li><a href="/index.php#main_skills">Skills</a></li>
            <li><a href="/index.php#main_value">Value</a></li>
            <li><a href="/index.php#main_all_pf">All portfolio</a></li>
            <li><a href="/index.php#main_about">About me</a></li>
          </ul>
        </li>
        <li>© birdman 2023</li>
        <li><a href="">Contact</a></li>
      </ul>
      <div class="resume">
        <p class="pc">Would you like to know more about me? <br>Click the button below to download my official <br>resume file. thank you.</p>
        <div class="down_btn">
          <span>Download file</span>
          <a href="" download>pdf.</a>
          <a href="" download>hwp.</a>
        </div>
      </div>
    </div>
  </div>
</footer>
<script>
  $(function(){
    ScrollTrigger.update();
    $(window).scroll(function(){
      AOS.refresh();
    });
  });
</script>